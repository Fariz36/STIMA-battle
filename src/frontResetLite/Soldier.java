...
public final class Soldier extends Robot {
  ...
  public Soldier(RobotController rc_) throws GameActionException {
    ...
    // Lite bot skips the scripted opening and uses the generic goal loop immediately.
    initialSoldiers = false;
    followID = -1;
    spawnTowerInfo = null;
  }
  ...
  protected void doMicro() throws GameActionException {
    ...
    switch (GoalManager.current().type) {
      case FIGHT_TOWER:
        ...
        if (rc.getHealth() < 45 && rc.senseNearbyRobots(rc.getType().actionRadiusSquared, TEAM).length == 0) {
  ...
  private void fightTowerLite() throws GameActionException {
    MapLocation towerLoc = GoalManager.current().target;
    if (rc.canSenseRobotAtLocation(towerLoc)) {
      goalTower = rc.senseRobotAtLocation(towerLoc);
    }

    if (rc.isActionReady() && Painter.paint(towerLoc)) {
      return;
    }
    ...
    if (rc.getHealth() < 45 && dist <= GameConstants.VISION_RADIUS_SQUARED) {
      Direction retreat = Pathfinding.getGreedyMove(
        rc.getLocation(),
        towerLoc.directionTo(rc.getLocation()),
        true,
        MovementManager.Mode.NO_ENEMY
      );
      if (retreat != null) {
        MovementManager.move(retreat);
      }
      return;
    }

    if (dist > rc.getType().actionRadiusSquared) {
      Direction advance = Pathfinding.getGreedyMove(
        rc.getLocation(),
        towerLoc,
        true,
        MovementManager.Mode.NO_ENEMY
      ...
      if (advance != null) {
        MovementManager.move(advance);
      }
    }

    if (rc.isActionReady()) {
      Painter.paint(towerLoc);
    }
  }
